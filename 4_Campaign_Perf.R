...
for( j in  3: length(validate_camp))
{
...
# dbGetQuery(dbCon, "sel TOP 10 * from AU_C2C_ENG")
  ...
  AND CAST(COLD.AUCT_START_DATE+INTERVAL'17'HOUR AS DATE)  BETWEEN CAST('@measureStart' AS DATE) AND CAST('@measureStart' AS DATE)+1
  ...
  AND 
  CAST(CREATED_TIME+INTERVAL'17'HOUR AS DATE) between CAST('@measureStart' AS DATE) AND CAST('@measureStart' AS DATE) +1 +30
  AND 
  CK_WACKO_YN='N'
  AND 
  ...
  WHERE    ACCT_TRANS_DT  between CAST('@measureStart' AS DATE)-1 AND CAST('@measureStart' AS DATE) + 1 +30-1
  ...
  write.csv(user_summary,paste("AU_C2C_ENGAGED_User_Summary_", validate_camp[j],".csv",sep=""),row.names = FALSE)
